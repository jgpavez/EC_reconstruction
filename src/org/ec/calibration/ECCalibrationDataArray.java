 ...
 *
 */
public class ECCalibrationDataArray extends ConcurrentHashMap<Integer, ECCalibrationData> implements
		Serializable
{
	...
	/**
	 * Calculates the Integer key used to get and put data in the HashMap.
	 * This key is calculated with the strip, layer, view and sector, with a simple 
	...
	{
		return new Integer(stripID + ( ECGeneral.MAX_STRIPS * layer.ordinal() ) + ( ECGeneral.MAX_STRIPS * 4 * view.ordinal() ) +
										( ECGeneral.MAX_STRIPS * 4 * 4 * ECGeneral.MAX_SECTORS ));
	}
	
	
	/**
	 * Get a CalibrationData object from the HashMap, converting the strip, layer, view and sector information
	 * into a key to search in the HashMap
	 * 
	 * @param stripID
	 * @param layer
	 * @param view
	...
	/**
	 * Put a CalibrationData object in the array, using the strip, layer, view and sector of the 
	 * calibration object to get the key to insert into the HashMap
	 * 
	 * @param input
	 * @return
	 */
