...
/**
 * The <code>ECGeometry</code> class, encapsulate the geometry data of EC for all
 * strips, layer, views and sectors.
 * Also calculate the default data for the calibration.
 * 
...
public class ECGeometry
{
	...
	/**
	 * Construct a ECGeometry object and define the default data for the calibration 
	 * object, the ideal geometry settings.
	 * 
	 */
	public ECGeometry()
	{
		...
		for ( int i = 0; i < ECGeneral.MAX_SECTORS; i++ ){
			bscSector[i].setX( bsc_r * Math.sin(bsc_a * d2Rad)*
										Math.cos(bscSector[i].getPhi() * d2Rad) + xoff[i]);
			bscSector[i].setY( bsc_r * Math.sin(bsc_a * d2Rad)*
										Math.sin(bscSector[i].getPhi() * d2Rad) + yoff[i]);
			bscSector[i].setZ( bsc_r * Math.cos(bsc_a * d2Rad) + zoff[i]);
		}
		
		zEChi		=	 bsc_r * Math.cos( bsc_a * d2Rad) - this.ylow
							* Math.sin(bsc_a * d2Rad);
		zEClow		=	 bsc_r * Math.cos( bsc_a * d2Rad) - this.yhi
							* Math.sin(bsc_a * d2Rad);
